...
function AboutSection() {
  ...
  const statHoverVariants = {
    hover: {
      scale: 1.1,
      boxShadow: "0 4px 16px rgba(91, 233, 255, 0.2)",
  ...
  const tagHoverVariants = {
    hover: {
      scale: 1.1,
      boxShadow: "0 2px 8px rgba(91, 233, 255, 0.2)",
      transition: { duration: 0.2 },
    },
  };
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 sm:px-6 z-10 flex flex-col items-center space-y-8">
        ...
        >
          <motion.div
            className="relative  w-full h-full bg-gradient-to-br from-gray-800 to-gray-900 rounded-full flex items-center justify-center border-2 border-[rgba(91,233,255,0.3)] overflow-hidden"
            whileHover={{
              scale: 1.05,
              boxShadow: "0 4px 16px rgba(91, 233, 255, 0.3)",
              borderColor: "rgba(91, 233, 255, 0.5)",
            }}
            transition={{ duration: 0.3 }}
          >
            <img
        ...
        >
          {stats.map((stat) => (
            <motion.div
              ...
              whileHover="hover"
            >
              <div className="text-xl sm:text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-500 to-cyan-100">
                {stat.value}
              </div>
        ...
        >
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {["Creative", "Reliable", "Innovative"].map((trait) => (
              <motion.span
                key={trait}
                className="px-2 py-1 text-xs font-semibold text-black bg-gradient-to-r from-cyan-500 to-cyan-100 rounded-full"
        ...
        <div className="md:flex gap-6 lg:gap-10">
          <motion.div
            ref={aboutCardRef}
            className="p-4 sm:p-6 rounded-lg bg-[rgba(31,41,55,0.3)] backdrop-blur-sm border-2 border-[rgba(91,233,255,0.3)] max-w-md mx-auto mb-6 md:mb-0"
            variants={hoverVariants}
            whileHover="hover"
          >
            <h2 className="text-xl sm:text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-500 to-cyan-100 mb-3">
              About Me
            </h2>
          ...
          <motion.div
            ref={skillsCardRef}
            className="p-4 sm:p-6 rounded-lg bg-[rgba(31,41,55,0.3)] backdrop-blur-sm border-2 border-[rgba(91,233,255,0.3)] max-w-md mx-auto"
            variants={hoverVariants}
            whileHover="hover"
          >
            <h3 className="text-xl sm:text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-500 to-cyan-100 mb-3 flex items-center gap-2">
              Skills & Technologies
            </h3>
              ...
              {skills.map((skill, index) => (
                ...
                >
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-white font-medium text-sm sm:text-base group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-cyan-500 group-hover:to-cyan-100">
                      {skill.name}
                    </span>
                  ...
                  <div className="w-full bg-gray-700 rounded-full h-2 overflow-hidden">
                    <motion.div
                      id={`skill-bar-${index}`}
                      className="h-full bg-gradient-to-r from-cyan-500 to-cyan-100 rounded-full"
                    />
                  </div>
                </motion.div>
              ))}
